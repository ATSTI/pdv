...
class AtsOrder:
    ...
    def order(self):
        ...
        sqld = 'SELECT m.CODMOVIMENTO, m.DATAMOVIMENTO, ' \
               'm.CODCLIENTE, m.STATUS, m.CODUSUARIO, v.CODVENDEDOR, ' \
               'm.CODALMOXARIFADO, DATEADD(3 hour to m.DATA_SISTEMA) ' \
               '  FROM MOVIMENTO m INNER JOIN VENDA v ON v.CODMOVIMENTO = m.CODMOVIMENTO ' \
               ' WHERE m.STATUS = 1 ' \
               '   AND m.DataMovimento > \'%s\'' %(hj)
